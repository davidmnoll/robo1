...
- Infrastructure-as-code for the hosted API/database lives under `terraform/`. It provisions Cloud SQL + App Engine Flexible inside the `robo1-489405` project (Terraform pins the Flex service to a single always-on instance for predictable cost; tweak `app_engine_max_instances` if you need to scale out).
...
- `terraform output -raw app_engine_url` returns the externally reachable API base (append `/api` when wiring the ROS bridge or dashboard).
