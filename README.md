...
- Lobby access keys are generated in the database. If no lobbies exist on startup (and a user exists), a "default" lobby is auto-created and its key is logged. Lobby owners can view their keys in the dashboard.
- The gateway can seed users, lobbies, and bots from JSON provided via `SEED_USERS_JSON` / `SEED_LOBBIES_JSON` / `SEED_BOTS_JSON`. The default compose file seeds `dmn322` / `TEST123!`, a `ros-core` lobby with an explicit `access_key`, and two bots (`bot_alpha`, `bot_beta`) that match the simulated robots. Example:

```bash
export LOBBY_KEY=super-secret
export SEED_USERS_JSON='[{"email":"dmn322","password":"TEST123!"}]'
export SEED_LOBBIES_JSON='[{"name":"ros-core","description":"Default ROS core lobby","access_key":"'"$LOBBY_KEY"'","owner_email":"dmn322","is_public":true}]'
...
```
- The `ros-core` service runs `rosbridge_server` plus a bridge node that pushes camera frames and opens an authenticated WebSocket to `/api/internal/ws/lobbies` for command delivery. Set `API_BASE_URL` (default `http://robot-gateway:8080/api`) and `LOBBY_KEY` (matching the lobby's `access_key` from the database) on the bridge so it can authenticate.
...
- `terraform output -raw app_engine_url` returns the externally reachable API base (append `/api` when wiring the ROS bridge or dashboard). When rolling out new App Engine versions, bump `app_engine_version_id` (e.g. `flex-v2`) so Terraform deploys a fresh version instead of reusing the previous one.
...
- To run the ROS bridge + Webots sim containers against the hosted App Engine API (instead of the local stack), set `CLOUD_RUN_LOBBY_KEY=<lobby access key>` and run `make cloud-sim`. If a Terraform state directory is present it automatically pulls the gateway URL; otherwise you can override with `CLOUD_RUN_API_URL=https://robot-gateway-.../api` (env var kept for backward compatibility). This uses `docker-compose.cloud.yml` to point `ros-core` and `sim` at the remote API so you can generate telemetry for an existing lobby.
- Prefer a manual App Engine Flexible deploy (outside Terraform)? Copy `api/app.flex.yaml` to `api/app.yaml`, fill in `env_variables` (Cloud SQL connection string, secrets, lobby key, CORS origins) plus `beta_settings.cloud_sql_instances`, then run `gcloud app deploy api/app.yaml`. Flex keeps at least one VM warm (≈$40+/mo) but allows aiortc to bind the UDP sockets WebRTC requires.
